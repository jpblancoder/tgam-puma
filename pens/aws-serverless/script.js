 ...
 * @return {String}
 */
function veMapSymbol(ticker, exchange) {
  let tick = ticker;
...
/**
 * @method scMapSymbol
 * @description Convert Barchart symbol to stockCalc symbol.
 * This logic must matches what is done in PageBuilder stockCalc feature.
 * @param {String} symbol - of stock
 * @param {String} exchange - of stock
 * @return {String}
 */
function scMapSymbol(symbol, exchange) {
  let ret = symbol;
  ret = ret.split(".")[0];
  ret = ret.replace("-", ".");
  return `${ret}:${exchange}`;
...
stocks.forEach(item => {
  item.stocks.forEach(stock => {
    let row = [];
    let veTick = veMapSymbol(stock.ticker, item.exchange_abbr);
    let scTick = scMapSymbol(stock.symbol, exchange[item.exchange_abbr]);
    ...
    // valuEngine PDF
    row.push(`${stock.ve === 200 ? success : failure}`);

    // valuEngine API (don't link)
    let veAPI = engine;
    veAPI = veAPI.replace(TICKER, veTick);
    veAPI = veAPI.replace(REGION, item.region);
    row.push(veAPI);

    // valuEngine AWS - Proxy API
    let veAWS = veAmazon;
    veAWS = veAWS.replace(TICKER, veTick);
    veAWS = veAWS.replace(REGION, item.region);
    veAWS = link(veAWS);
    row.push(veAWS);

    // stockCalc PDF
    row.push(`${stock.sc === 200 ? success : failure}`);

    // stockCalc AWS - Proxy API
    let scAWS = scAmazon;
    scAWS = scAWS.replace(TICKER, scTick);
    scAWS = link(scAWS);
    row.push(scAWS);
